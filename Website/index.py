...
def index():
    ...
    if request.method == 'POST':
        ...
        # #Generar el gráfico si hay menos de 10 alertas
        # if len(alertas['Name']) < 10 and isinstance(order, pd.DataFrame) and not order.empty:
        #     plot_url = generate_plot(order, data_observations, time_array, limite_altitud, observer, date_i)
...
# def generate_plot(order, data_observations, time, limit, observer, date_i):
...
#     # Agrupar datos por etiqueta para el gráfico
#     graphic = []
#     for label in orderlabels:
#         graphic.append(data_observations[data_observations['Label'] == label])
    ...
    #         ALT = each['Alt'].tolist()
    #         AZ = each['Az'].tolist()
    #         T = [t.datetime.hour - 24 if 13 <= t.datetime.hour <= 23 else t.datetime.hour 
    #             for t in each['Time']]
    ...
    # # Líneas y detalles del gráfico
    # plt.plot([t.datetime.hour - 24 if 13 <= t.datetime.hour <= 23 else t.datetime.hour 
    #          for t in time], [limit] * len(time), '--', color='grey')
    
    # plt.colorbar().set_label("Azimuth [deg]")
    # plt.xlabel("Hours around Midnight")
    # plt.ylabel("Altitude [deg]")

    # # Líneas de tiempo de observación
    # for each in ordertime:
    #     plt.plot([each[0]] * 2, [min(min_alt), max(max_alt)], '--', color='orange')
    #     plt.plot([each[1]] * 2, [min(min_alt), max(max_alt)], '--', color='salmon')

    # # Ejes y título
    # hours = [str(round(num + 24, 1)) if num < 0 else str(round(num, 1)) 
    #         for num in np.array([-6, -4, -2, 0, 2, 4, 6])]
    # plt.xticks(np.array([-6, -4, -2, 0, 2, 4, 6]), hours)
    # plt.title(f'Starting time {date_i}\nLocation: ({observer[0]}, {observer[1]})')
